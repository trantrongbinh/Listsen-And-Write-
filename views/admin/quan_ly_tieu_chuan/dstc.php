...
$dstcvnc1 = $tcvnc1->layTieuChuan1();

if(isset($_POST['submit1'])){
  $box=$_POST['num'];
  while (list($key, $val) = @each($box)){
    $ketqua = $tcvnc1 ->xoaTieuChuan1($val);
  }
  ?>
...
<h2><strong>Quản lý tieu chuẩn</strong></h2>        
<form name="form1" action="" method="POST">
<div style="width:600px; height:120px;margin: 15px 10px 10px 10px;"> 
  <label> Tieu Chuan: </label>  
  <input type="text" name="tieuchuan" value="" style="width:480px; height:24px;margin-bottom:5px;">
  <br>
...
<table>
  <tr>
    ...
    <?php
      $stt = 1;
      foreach ($dstcvnc1 as $ds) { 
      ?>
          <tr>
            <td style="text-align:center;"><?=$stt++?></td>
            <td style="text-align: center;"><?=$ds['TenTCVN']?></td>
            <td style="text-align: center;"><i class="fa fa-pencil fa-fw"></i> <a href="?p=edit-tc&id-tc=<?=$ds['idc1']?>">Edit</a></td>
...
function them() {
    window.location.assign("?p=them-tieu-chuan")
}
</script>