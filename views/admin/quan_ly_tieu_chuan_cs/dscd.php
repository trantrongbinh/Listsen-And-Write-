...
if(isset($_POST['submit1'])){
  ...
  while (list($key, $val) = @each($box)){
    $ketqua = $tccs1 ->xoaTieuChuanCS1($val);
  }
  ?>
...
<table>
  <tr>
    ...
    <?php
      $stt = 1;
      foreach ($dstccs1 as $ds) { 
      ?>
          <tr>
            <td style="text-align:center;"><?=$stt++?></td>
            <td style="text-align: center;"><?=$ds['ChuDe']?></td>
            <td style="text-align: center;"><i class="fa fa-pencil fa-fw"></i> <a href="?p=edit-cd-tccs&id-tccs1=<?=$ds['idc1']?>">Edit</a></td>
...
function them() {
    window.location.assign("?p=them-chu-de-tccs")
}
</script>